...
$koneksi = mysqli_connect('localhost', 'root', 'root', 'pendidikan');
...
if ($request == 4) {
    ...
    $jk = $data->jk;
    $prodi = $data->prodi;
    $sql = mysqli_query($koneksi, "INSERT INTO mahasiswa (nim, nama, jk, prodi) VALUES ('$nim', '$nama', '$jk', '$prodi')");
    if ($sql) {
...
if ($request == 3) {
    ...
    $jk = $data->jk;
    $prodi = $data->prodi;
    $sql = mysqli_query($koneksi, "UPDATE mahasiswa SET nim='$nim', nama='$nama', jk='$jk', prodi='$prodi' WHERE id=" . $id);
    if ($sql) {
